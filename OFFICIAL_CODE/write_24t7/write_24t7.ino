#include <SPI.h>          // Giao tiếp SPI với MCP2515
#include <mcp2515.h>      // Thư viện MCP2515
#include <Keypad.h>       // Đọc ma trận bàn phím
...
void loop() {
  ...
  // // Bit 32 → Trạng thái phanh: 1 = có phanh , 0 = không phanh (mặc định)
  // //engMsg.data[4] |= ( << 0); // mặc định
  // if (bitState[5]) {
  //   engMsg.data[4] |= (1 << 0); // đang phanh
  // }
  ...
  // while (mcp2515.readMessage(&letter)  == MCP2515::ERROR_OK){
  //   if(letter.can_id == 0x234){
  //      Serial.print("HMI:  ");
  //     for (int i = 0; i < 8; i++) {
  //         if (letter.data[i] < 0x10) Serial.print("0"); // thêm số 0 phía trước nếu < 0x10
  //         Serial.print(letter.data[i], HEX);
  //         Serial.print(" ");
  //       }
  ...
  //     Serial.print("ID :"); Serial.println(letter.can_id);
  //   }
  // }

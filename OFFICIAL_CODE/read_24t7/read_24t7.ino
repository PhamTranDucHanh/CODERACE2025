...
// Hàm đọc từng bit trong mảng dữ liệu
...
void loop() {
  ...
  while (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
    ...
    //else{
    //   //các message không liên quan cho nó ra khỏi hàng liền.
    //   Serial.println("Unknown message..: ");
    // }

    // In từng bit theo thứ tự bit 0 nằm bên trái nhất trong mỗi byte
  ...
  if (canMsg.can_id == 0x17C) {
    ...
    // 2. Cập nhật engineOn theo ngưỡng 100 rpm
    if (engineSpeed >= 100) {
      engineOn = true;
    } else {
      engineOn = false;
  ...
  // ======= DÒNG 3: IN TRẠNG THÁI NGƯỜI ĐỌC HIỂU =======
  // if (gotCVT || gotENG) {
  //   Serial.print("GEAR: ");
  //   switch (gearState) {
  ...
  //   Serial.print("    Pre_Progress: ");
  //   Serial.print(enginePreProgress);

  //   Serial.print("    Is_Progress: ");
  //   Serial.println(engineProgress);
  //   Serial.println("----------------------------------------");
    
  // }

    // Trường hợp 1: Vừa khởi động nhưng vào D sớm
    if(lock){
  //     if (engineOn && gearState == 0 && pedalBrake && parkBrake){    // Động cơ bật
  //     // if (gearState == 0 && pedalBrake){    // Xe vừa khởi động
  //     //   if(parkBrake){   // Phanh tay chưa thả
  //         displayMessage1 = ">>> Realease the ";
  ...
  //         lastDisplayTime = now;
  //       // }
  //     //   else{  //Phanh tay đã thả => Chuẩn bị lái xe
  ...
  //   }
  // // else{ // Động cơ tắt
  //   if(!engineOn && gearState == 0 && parkBrake == 1){ // Cần số là P
  //     // if(parkBrake){ // Phanh tay đã kéo
  //       displayMessage1 = "Correctly parked";
  //       displayMessage2 = "";
  //       displayMessage3 = ""; 
  //       drawOLED();
  //       lock = 0;
  ...
  //     // }
  //   }
  //   if(engineOn == 0 && gearState == 0 && parkBrake == 0){
  //     displayMessage1 = ">> Engage the Parking";
  //     displayMessage2 = "Brake";
  //     displayMessage3 = ""; 
  //     drawOLED();
  ...
  //   }
  //   // else{ // Cần số khác P
  //     if(engineOn == 0 && gearState != 0 && parkBrake == 1){ // Phanh tay đã kéo
  ...
  //     }
  //     // else{   // Phanh tay chưa kéo
  //       if(engineOn == 0 && gearState != 0 && parkBrake == 0 && pedalBrake == 1){ // Phanh chân đã đạp
  ...
  //     if(engineOn == 0 && gearState != 0 && parkBrake == 0 && pedalBrake == 0){
  //       displayMessage2 = ">>> Shift to P or";
  //       displayMessage3 = ">>> Hold Brake";
  //       Serial.println(displayMessage1); 
  //       drawOLED();
  //       lock = 0;
  //       warning = true;   
  //       lastDisplayTime = now;
  //     }
      // else{ // Phanh chân chưa đạp => có thể xe trôis
...
// Sau 3 giây thì xóa cảnh báo 
  ...
  // delay(500);  // Đợi 1 chút trước khi đọc tiếp
  if(warning){
    // sendCAN;
